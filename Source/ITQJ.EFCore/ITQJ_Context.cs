namespace ITQJ.EFCore
{
    ...
    public class ITQJ_Context : DbContext
    {
        public ITQJ_Context(DbContextOptions<ITQJ_Context> options) : base(options)
        {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<User>().HasMany(m => m.Postulants);

            modelBuilder.Entity<Project>().HasMany(m => m.Postulants);

            modelBuilder.Entity<Message>().HasOne(m => m.User).WithMany(m => m.Messages).HasForeignKey(r => r.UserId);

            modelBuilder.Entity<Message>().HasOne(m => m.Project).WithMany(m => m.Messages).HasForeignKey(r => r.ProjectId);

            modelBuilder.Entity<Review>().HasOne(m => m.User).WithMany(m => m.Reviews).HasForeignKey(r => r.UserId);
